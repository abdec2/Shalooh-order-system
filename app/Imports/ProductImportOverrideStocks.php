...
class ProductImportOverrideStocks implements ToCollection, WithHeadingRow
{
    /**
    ...
    */
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row) 
        {
            ...
            //             $updateInventory->quantity = (int)$row['qty'];
            ...
            $bin1 = Bins::where('product_id', 1)->with('Inventory')->get();
            if(count($bin1) > 0) 
            {
                $avail_qty = 0;
               foreach($bin1 as $item){
                  $avail_qty = (int)$avail_qty + (int)$item->Inventory[0]->quantity;
               }
            }
